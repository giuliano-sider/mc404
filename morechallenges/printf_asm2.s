...
@ this changes the number into the quotient of the division. numsize is also adjusted. 
	...
	push { r7-r9 }
	mov \Rremainder, 0
	lsr r7, \Rnumsize, 1 @ we will index by halfwords, as that is how the division is done.
DivideTheNumber\@:
	cbz r7, DoneDividing\@
	sub r7, 1 @ look at next less significant halfword 
	ldrh r8, [\Rnumberptr, r7 lsl 1]
	add r8, \Rremainder, lsl 16 @ "double halfword" dividend, (at most) halfword divisor
...
AdjustNumSize:\@
	...
	cbnz r8, AdjustedNumSize\@ @ while word is zero, decrement numsize.
...
.macro Macro_PrintString Rstring @ dont use r2-r3
	...
	PrintStringInNormalOrder\@:
		...
		cbz r3 FinishedPrintString\@
			...
			bl PrintChar
			pop { r0 }
		b PrintThisStringInNormalOrder
	FinishedPrintString\@:
	pop { r3 }
.endm

.macro Macro_ParseStringAsDecimalNumber Rstring, Rindex, Rnumber, Rscratch, Rscratch2
	@ string, index (gets incremented up to first non-digit), variable to keep the number, scratch register 1 & 2
	mov Rnumber, 0 @ initialize number = 0
ParseStringAsDecimalNumberLoop\@:
	ldr \Rscratch, [\Rstring, \Rindex]
	cmp \Rscratch, '0'
	blt DoneParseStringAsDecimalNumber\@
	cmp \Rscratch, '9'
	bgt DoneParseStringAsDecimalNumber\@
	sub \Rscratch1, '0' @ turn it from ascii to the actual number
	add \Rscratch2, \Rscratch1, \Rnumber, lsl 1
	add \Rnumber, \Rscratch2, \Rnumber, lsl 3 @ number = number*10 + asciitoint(string[index])
	add \Rindex, 1
	b ParseStringAsDecimalNumberLoop\@
DoneParseStringAsDecimalNumber\@:
.endm

.macro Macro_ParseStringAsOctalNumber Rstring, Rindex, Rnumber, Rscratch, Rscratch2
...
.macro Macro_ParseStringAsHexNumber Rstring, Rindex, Rnumber, Rscratch, Rscratch2
	@ string, index (gets incremented up to first non-digit), variable to keep the number
mov \Rnumber, 0 @ initialize number = 0
...
.byte (ReadFormatArg-BranchByState)/2
	...
	LengthSpecH:
		...
		cbz regvar_lengthspec, InvalidLengthModifier
		add regvar_i, 1 @ i++
		b LoopThroughFormatString
	LengthSpecL:
	...
	ReadPercent: @ just print a '%'. if there's extraneous information in the specifier,
		...
		cbnz regvar_width, UselessInfoWithPercentSpecifier @ send error
		cmp regvar_lengthspec, 4 
		bne UselessInfoWithPercentSpecifier
		cbnz regvar_flags1, UselessInfoWithPercentSpecifier
	...
	ReadString:
		add regvar_i, 1 @ i++ to consume the specifier character
		cbnz regvar_flags1, InvalidFlagsWithString @ if any of the '0', ' ', '+', '#' flags are set
		cmp regvar_lengthspec, 4
		bne InvalidLengthModifierWithString
		...
		cbnz r1, DashFlagIsSet @ if dash flag is unset, field is right justified within width
			...
			b LoopThroughFormatString
		DashFlagIsSet:  @ field is left justified within width
			bl SwitchToPrintProfile
			Macro_PrintString r0 @ string is at r0
			Macro_PrintSomeCharNTimes r12, r3 @ r3 is width-len (blanks to print)
			b LoopThroughFormatString
	ReadCharacter: @ very similar to string; handling them together, in assembly language, though, would be a pain
		add regvar_i, 1 @ i++ to consume the specifier character
		cbnz regvar_flags1, InvalidFlagsWithChar @ if any of the '0', ' ', '+', '#' flags are set
	...
	ReadSpecifierN:
		...
		cbnz regvar_width, InvalidModifiersWithSpecifierN
		cmp regvar_lengthspec, 4
	...
	ReadUnsignedDecimal:
		ldr regvar_state, =DigitsLookup 
	@ look up table of digits(not really necessary for decimal, helpful for hex)lengthspec can be safely clobbered now
		add regvar_i, 1
		mov r0, regvar_lengthspec
		bl ObtainValueFromNextArg @ returns pointer to memory block where the number is kept
	...
	ReadSignedDecimal:
		...
		cbz r12, SurelyAPositiveNumber @ if zero it is a positive number
...
@ copies our number into internal buffer(placed in r0); ceil(numberbytes/4) words in little endian form
	...
	ActuallyFindingDigits:
		...
		Macro_DivVectorByInt16 r0, r1, r3, r2 @ r0->number, r1->numsize(words), r3->divisor, r2->remainder(output)
			@ this changes the number into the quotient of the division. numsize is also adjusted.
		ldr regvar_lengthspec, [regvar_state, r2] @ temp = LookUpTable[remainder] (clobber ok, i promise)
		str regvar_lengthspec, [freereg, r12] @ outputstring[outputstringlength] = temp
	...
	DoneDecodingNumber: @ now print it
		...
		bne LookAtPoundFlag @ if not equal to zero, we examine if we should add a '0' or '0x' or '0X' prefix
			mov r2, '0' @ r2 = '0'
			str r2, [freereg] @ outputstring[0] = '0'
		...
		LookAtPoundFlag:
			...
			itttt eq
			moveq r2, '0'
			streq r2, [freereg, r12] @ outputstring[outputstringlength] = '0' // octal prefix
			addeq r12, 1 @ outputstringlength++
			beq NowLookAtSign @ now we are only left with the x and X cases
			ldr r2, [regvar_state, 17] @ r2 = LookUpTable(Caps|NoCaps) [17] @ has x or X depending on table
			str r2, [freereg, r12] @ outputstring[outputstringlength] = x or X
			add r12, 1 @ outputstringlength++
			mov r2, '0'
			str r2, [freereg, r12] @ outputstring[outputstringlength] = '0'
			add r12, 1 @ outputstringlength++
	NowLookAtSign:
		and r2, regvar_flags2, (1<<24) @ test the negative sign bit for this number
		cbz r2 NumberIsNotNegative
		mov r2, '-'
		str r2, [freereg, r12] @ outputstring[outputstringlength] = '-'
		add r12, 1 @ outputstringlength++
		b PrepareToPrint
	NumberIsNotNegative:
		and r2, regvar_flags1, (1<<24) @ if flags.plus == true, then print a '+'
		it ne @ flag set
		mov r2, '+'
		str r2, [freereg, r12] @ outputstring[outputstringlength] = '+'
		add r12, 1 @ outputstringlength++
		b PrepareToPrint
		and r2, regvar_flags1, (1<<8) @ if flags.space == true, then print a 'space'
		it ne @ flag set
		mov r2, ' '
		str r2, [freereg, r12] @ outputstring[outputstringlength] = ' '
		add r12, 1 @ outputstringlength++
...
FinishedReadingFormatString:
	cbnz regvar_state, FormatStringPrematurelyEnded 
	...
	mov r1, 0
	str r1, [regvar_buffer, regvar_buffercount] @ buffer[buffercount] = \0
...
push {  r1-r3 , regvar_argstr, regvar_j, regvar_args, regvar_arg, regvar_argstate, lr }
	...
	cbnz r1, MissingArguments @ if exhaustedargs == true, error
	mov regvar_argstate, matchleftbracket @ the first part of the argument is the [optional] left bracket
	
ReadTheArgumentString:
	ldr r1, =ReadArgumentStringBranchTable @@@@
	ldr r2, [regvar_formatstr, regvar_j] 
	tbb [r1, r2] @ switch(formatstr[j])

	HandleLeftBracket: @ '[' in matchleftbracket->ok. otherwise, error: misplaced left bracket in argstring
	...
	HandleR: @ 'r': start of register specification. otherwise invalid
		...
		add regvar_j, 1 @ j++
		Macro_ParseStringAsDecimalNumber regvar_argstr, regvar_j, r1, r2, r3 @ number in r1. r2 and r3: scratch.
	...
	Handle123456789: @ decimal constant detected
		...
		bne SyntaxErrorInArgumentString
		Macro_ParseStringAsDecimalNumber regvar_argstr, regvar_j, r1, r2, r3 @ result in r1. r2-r3: scratch
	...
	Handle0: @ octal or hex constant detected
		...
		bne SyntaxErrorInArgumentString
		add regvar_j, 1
		ldr r1, [regvar_argstr, regvar_j] @ if argstr[j] == 'x' or 'X' we have a hex constant. otherwise: octal
		cmp r1, 'x'
		beq TreatHexConstant
		cmp r1, 'X'
		beq TreatHexConstant
		sub regvar_j, 1
		Macro_ParseStringAsOctalNumber regvar_argstr, regvar_j, r1, r2, r3 @ result in r1. r2-r3: scratch
		str r1, [regvar_args, regvar_arg] @ args[arg] = constant read from string
		b NextStateObtainValueFromNextArg @ handle next state transition
	TreatHexConstant:
		add regvar_j, 1
		Macro_ParseStringAsHexNumber regvar_argstr, regvar_j, r1, r2, r3 @ result in r1. r2-r3: scratch
		str r1, [regvar_args, regvar_arg] @ args[arg] = constant read from string
		b NextStateObtainValueFromNextArg @ handle next state transition
	...
	HandleComma: @ ',' in matchsign, matchshift, matchrightbracket->ok. otherwise, error
		tbb [pc, regvar_argstate] @ switch(argstate)
	JumpToHandleBracket:
		.byte (PrematurelyFinishedArgumentString-AreBracketsClosed)/2
		.byte (PrematurelyFinishedArgumentString-AreBracketsClosed)/2
		.byte (AreBracketsClosed-AreBracketsClosed)/2
		.byte (AreBracketsClosed-AreBracketsClosed)/2
		.byte (AreBracketsClosed-AreBracketsClosed)/2
		.byte (PrematurelyFinishedArgumentString-AreBracketsClosed)/2
	AreBracketsClosed:
...
FinishedReadingArgument:
	...
	it eq
	strbeq r1, [regvar_args] @ store value calculated
	cmp r0, 2 @ if numbytes==2, store halfword:
	it eq
	strheq r1, [regvar_args] @ store value calculated
...
LinkRegisterInvalid: @ called from within ObtainValueFromNextArg (extra stack unwinding)
	...
	b FatalErrorUnwind
LinkRegisterInvalidMsg: .asciz "Error: Link Register is inaccessible to printf, clobbered by branch and link\n"
...
MissingArguments: @ called from within ObtainValueFromNextArg (extra stack unwinding)
	...
	b FatalErrorUnwind
MissingArgumentMsg: .asciz "Error: missing necessary arguments in the argument string\n"
...
SyntaxErrorInArgumentString: @ called from within ObtainValueFromNextArg 
	...
	b FatalErrorUnwind
SyntaxErrorInArgumentStringMsg: .asciz "Error: misplaced sign in the argument string\n"
...
@@@@ must be in 'print' register profile !!! (prematurely evil optimization ???)
	...
	mov regvar_flags2, 0 @ clear the other flags
	mov width, 0 @ default: no minimum field width
	mov lengthspec, 4 @ default length of number: 4 bytes
...
StillSpaceInBuffer:
	str r0, [regvar_buffer, regvar_buffercount]  @ buffer[buffercount] = character to print
...
@ characters immediately and the peripheral handles the rest without blocking the CPU)
	.byte (FinishedPrintf-@@@@HandleDashFlag)/2 @ nul terminator of the format string
	.rept '%'
		.byte (PrintNormalCharacter-@@@@HandleDashFlag)/2
	.endr
	.byte (BeginReadingFormatSpecifier-@@@@HandleDashFlag)/2
	.rept 255 - '%'
		.byte (PrintNormalCharacter-@@@@HandleDashFlag)/2
	.endr
...
WidthBranchTable: @ streamlines the code. obsviously not essential
	.byte (FormatStringPrematurelyEnded-@@@@HandleDashFlag)/2 @ \0 terminator
	.rept '*' - 1 @ fill up to '*'
		.byte (NoWidth-@@@@HandleDashFlag)/2
	.endr
	.byte (HandleWidthArg-@@@@HandleDashFlags)/2
	.rept '0'-'*'-1 @ fill between asterisk and 0
		.byte (NoWidth-@@@@HandleDashFlag)/2
	.endr
	.rept 10 .byte (HandleWidthSpec-@@@@HandleDashFlags)/2 .endr
...
LengthSpecBranchTable @ streamlines the code. obsviously not essential
	.byte (FormatStringPrematurelyEnded-@@@@HandleDashFlag)/2 @ \0 terminator
	.rept '0' - 1
		.byte (NoLengthSpec-@@@@HandleDashFlag)/2
	.endr
	.rept 10 .byte (LengthSpecNumber-@@@@HandleDashFlag)/2 .endr
	.rept 'h' - '9' - 1
		.byte (NoLengthSpec-@@@@HandleDashFlag)/2
	.endr
	.byte (LengthSpecH-@@@@HandleDashFlags)/2
	.rept 'l'-'h'-1 
		.byte (NoLengthSpec-@@@@HandleDashFlag)/2
	.endr
	.byte (LengthSpecL-@@@@HandleDashFlags)/2
